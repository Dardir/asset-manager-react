...
class CorMaintOccGraph extends PureComponent {
  ...
  getDataSets() {
    let allEquipmentTypes = this.props.correctiveMaintenances.map(function (currentMaintenance) {
    ...
    let sets = uniqueEquipmentTypes.map(function (eqType) {
      ...
      for(let i=1; i<=12; i++) {
        let count = 0;
        let maintListFilteredPerMonthPerType = allCorrectiveMaintences.filter(function (maintenance) {
