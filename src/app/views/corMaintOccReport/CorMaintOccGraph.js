...
class CorMaintOccGraph extends PureComponent {
  constructor(props) {
    super(props);
  ...
  getDataSets() {
    let allEquipmentTypes = this.props.correctiveMaintenances.allinstances.map(function (currentMaintenance) {
      return currentMaintenance.typeId;
    });
    let uniqueEquipmentTypes = this.uniq(allEquipmentTypes);
    let allCorrectiveMaintences = this.props.correctiveMaintenances;
    let sets = uniqueEquipmentTypes.map(function (eqType) {
      let countList=[];
      // looping over the 12 months
      for(let i=1; i<=12; i++) {
        let count = 0;
        let maintListFilteredPerMonthPerType = allCorrectiveMaintences.allinstances.filter(function (maintenance) {
      ...
      <AnimatedView>
        ...
        <div className="row">
          <div className="col-xs-12">
            <Panel
              title="Corrective Maintenance Per Equipment Types"
              hasTitle={true}
...
export default CorMaintOccGraph;
